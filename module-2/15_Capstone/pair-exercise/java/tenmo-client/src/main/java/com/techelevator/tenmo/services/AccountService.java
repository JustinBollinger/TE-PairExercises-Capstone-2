...

{
	...
	public BigDecimal getBalance(AuthenticatedUser currentUser)
	{
		...
		try 
		{
			balance = restTemplate.exchange(BASE_URL + "/users/accounts/" + currentUser.getUser().getId(), 
	...
	public Integer getAccountId(AuthenticatedUser currentUser)
	{
		...
		try
		{
			accountId = restTemplate.exchange(BASE_URL + "/users/accounts/ids" + currentUser.getUser().getId(), 
