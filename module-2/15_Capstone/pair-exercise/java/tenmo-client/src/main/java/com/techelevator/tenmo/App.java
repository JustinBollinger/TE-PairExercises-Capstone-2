...
public class App
{
		...
		private static void getTransferTypeStatusDetails(Transfer[] transferArray, String[] transferString, 
				String[] transferType,
		...
		private static void sendBucks() 
		{
			...
			System.out.println("How much TEnmo Bucks would you like to send?");
			...
			System.out.println("Who would you like to send yout TE Bucks to?");
			...
			try
			{
				Transfer transfer2 = restTemplate
						.exchange(API_BASE_URL + "new-transfer", HttpMethod.POST, entity2,
			...
			
			{
				...
				System.out.println("Cannot process this transfer. Please try again.");
				mainMenu();
			}
			System.out.println("Your transfer complete.");
		}

		private static User viewAndSelectUsers() 
		{
			HttpEntity entity = getEntity();
			User[] userArray = restTemplate.exchange(API_BASE_URL + "users", HttpMethod.GET, entity, 
					User[].class)
					.getBody();
			...
			User recipient = restTemplate
					.exchange(API_BASE_URL + "user-names/?name=" + chosen, HttpMethod.GET, entity, 
							User.class).getBody();
			return recipient;
		}
		...
		private static void approveTransfer(Transfer transfer) 
		{
			...
			Transfer transferComplete = restTemplate
					.exchange(API_BASE_URL + "new-transfer/approved", HttpMethod.PUT, entityComplete,
							Transfer.class)
					.getBody();
		}
