...
public class TransferService
{
		...
		try
		{
...

			{
				...
				for (Transfers i : output)
				{
					...
					{
						Transfers temp = restTemplate.exchange(BASE_URL + "transfers/" + i.getTransferId(),
								HttpMethod.GET, makeAuthEntity(), Transfers.class).getBody();
...

	{
		...
		{
			Scanner scanner = new Scanner(System.in);
			users = restTemplate.exchange(BASE_URL + "listusers", HttpMethod.GET, makeAuthEntity(), User[].class)
					.getBody();
			System.out.println("-------------------------------------------\r\n" + "Users\r\n" + "ID\t\tName\r\n"
					+ "-------------------------------------------");
			for (User i : users)
...

			{
				...
				String output = restTemplate
						.exchange(BASE_URL + "transfer", HttpMethod.POST, makeTransferEntity(transfer), String.class)
						.getBody();
...

	{
...

		{
...

			{
				...
				String output = restTemplate
						.exchange(BASE_URL + "request", HttpMethod.POST, makeTransferEntity(transfer), String.class)
						.getBody();
				System.out.println(output);
			}
		}
...

	{
		...
		try
		{
			output = restTemplate.exchange(BASE_URL + "request/" + currentUser.getUser().getId(), HttpMethod.GET,
					...
					if (i.getAccountTo() != currentUser.getUser().getId())
					{
						if (Integer.parseInt(input) == i.getTransferId())
						{
...

							{
								...
								{
									results = restTemplate.exchange(BASE_URL + "transfer/status/" + id, HttpMethod.PUT,
											makeTransferEntity(i), String.class).getBody();
	...
	public User[] getUsers()
	{
		...
		try
		{
			user = restTemplate.exchange(BASE_URL + "listusers", HttpMethod.GET, makeAuthEntity(), User[].class)
					.getBody();
			for (User i : user)
			{
